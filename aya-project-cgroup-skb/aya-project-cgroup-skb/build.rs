...
/// This crate has a runtime dependency on artifacts produced by the `aya-project-cgroup-skb-ebpf` crate.
...
/// crate's undeclared dependency on artifacts from `aya-project-cgroup-skb-ebpf` would cause build (and
...
/// `aya-project-cgroup-skb-ebpf`.
///
/// Note that this solution is imperfect: in particular it has to balance correctness with
/// performance; an environment variable is used to replace true builds of `aya-project-cgroup-skb-ebpf`
/// with stubs to preserve the property that code generation and linking (in
/// `aya-project-cgroup-skb-ebpf`) do not occur on metadata-only actions such as `cargo check` or `cargo
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "aya-project-cgroup-skb-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `aya-project-cgroup-skb-ebpf`, so cargo will automatically rebuild us
        // if `aya-project-cgroup-skb-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `aya-project-cgroup-skb-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("aya-project-cgroup-skb-ebpf");
